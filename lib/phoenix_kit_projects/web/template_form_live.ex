defmodule PhoenixKitProjects.Web.TemplateFormLive do
  ...
  defp maybe_subscribe_translations(socket) do
    if Phoenix.LiveView.connected?(socket) and Translations.available?() and
      ...
      Translations.subscribe("template", socket.assigns.project.uuid)
    end
  ...
  def handle_info(
    ...
    if Map.get(payload, :empty, false) do
      ...
      # `payload.fields` instead of `Projects.get_project/1`.
  ...
  # shared topic, stale events) — the form only cares about its own template.
